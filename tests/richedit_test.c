...
int main(void)
{
    ...
    /* ---- EM_SETRECT: the box the text is laid out in ---- */
    {
         ...
         * is the point. */
        {
            ...
            /* **This asserted one pass and now asserts two, in the commit
             * that made it two.** That is the whole point of the check: the
             * cost of changing `rich_fmt` shows up here, in the diff, instead
             * of as nine red pixels in another repository's ruler tests.
             *
             * It is two because the mapping from `rect.left` to where
             * character 0 lands is discontinuous at the first rectangle --
             * `inset + selection bar` before it, `rect.left - 1` after --
             * so a pass that measures under one mapping and applies under the
             * other lands nine short. Every later pass is inside one regime
             * and converges at once, which is why it is two and not many.
             *
             * **wordpad was changed to converge rather than assume**
             * (`dan/converge`, bounded at three), and the two landed
             * together. A library change that moves where text sits is not
             * mergeable on its own suite alone -- this file learned that by
             * having the previous attempt reverted. */
            CHECK(passes <= 2,
                  "and it takes two, because the mapping changes at the "
                  "first rectangle: a consumer that assumes one pass lands "
                  "nine pixels short until the second");
        }
